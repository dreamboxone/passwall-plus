...
# ever seen work.
PWPLUS_ETC="${PWPLUS_ETC:-/etc/passwall-plus}"
PWPLUS_RUN="${PWPLUS_RUN:-/var/run/passwall-plus}"
PWPLUS_OWN_DIR="${PWPLUS_OWN_DIR:-/usr/libexec/passwall-plus}"
PWPLUS_LIB="${PWPLUS_LIB:-/usr/libexec}"
...
PWPLUS_CANDIDATES="$PWPLUS_RUN/candidates.tsv"
PWPLUS_RANKED="$PWPLUS_RUN/ranked.tsv"
PWPLUS_STATUS="$PWPLUS_RUN/status"
PWPLUS_MESSAGE="$PWPLUS_RUN/message"
PWPLUS_PROGRESS="$PWPLUS_RUN/progress"
PWPLUS_BEST="$PWPLUS_ETC/best.json"
PWPLUS_BEST_META="$PWPLUS_ETC/best.meta"
PWPLUS_NODE_CACHE="$PWPLUS_ETC/nodes.cache"
PWPLUS_CONFIG_JSON="${PWPLUS_CONFIG_JSON:-/var/etc/passwall-plus.json}"
PWPLUS_BRIDGE_JSON="${PWPLUS_BRIDGE_JSON:-/var/etc/passwall-plus-bridge.json}"
PWPLUS_VERSION_FILE="$PWPLUS_ETC/version"
...
say_message() {
	echo "$*" > "$PWPLUS_MESSAGE" 2>/dev/null || true
	return 0
}

clear_message() {
	rm -f "$PWPLUS_MESSAGE" 2>/dev/null || true
	return 0
}

say_status() {
	echo "$1" > "$PWPLUS_STATUS" 2>/dev/null || true
	return 0
}
...
cfg() {
	...
	if command -v uci >/dev/null 2>&1; then
		_v="$(uci -q get "passwall-plus.config.$1" 2>/dev/null)" || _v=""
	fi
	[ -n "$_v" ] || _v="$2"
	echo "$_v"
	return 0
...
pwplus_version() {
	cat "$PWPLUS_VERSION_FILE" 2>/dev/null || echo unknown
	return 0
}
...
xray_paths() {
	echo "$PWPLUS_OWN_DIR/xray /usr/bin/xray /usr/local/bin/xray"
	return 0
}
...
find_xray() {
	...
	# was built to look - and ours are in /etc/passwall-plus/geo, so a configuration
...
space_for() {
	...
	if [ "$_free" -lt $((_need + PWPLUS_SPACE_MARGIN)) ]; then
		say_message "Not enough room in $_dir: needs $(human_size $((_need + PWPLUS_SPACE_MARGIN))), $(human_size "$_free") free"
		warn "refusing download: needs $(human_size $((_need + PWPLUS_SPACE_MARGIN))) in $_dir, $(human_size "$_free") free"
		return 1
	fi
	return 0
}
...
PWPLUS_GEO_SEARCH="${PWPLUS_GEO_SEARCH-/usr/share/xray /usr/local/share/xray /usr/share/v2ray}"

geo_dir() {
	_d="$(cfg geo_dir "$PWPLUS_ETC/geo")"
	for _c in "$_d" $PWPLUS_GEO_SEARCH; do
		[ -n "$_c" ] || continue
...
nft_probe() {
	...
	if printf 'table inet pwplus_probe {\n chain c {\n  type filter hook prerouting priority mangle; policy accept;\n  %s\n }\n}\n' "$_rule" | nft -f - >/dev/null 2>&1; then
		nft delete table inet pwplus_probe >/dev/null 2>&1 || true
...
ipt_probe() {
	_cache="$PWPLUS_RUN/caps.ipt_tproxy"
	if [ -f "$_cache" ]; then
		[ "$(cat "$_cache" 2>/dev/null)" = "1" ] && return 0
		return 1
	fi
	if iptables -t mangle -N pwplus_probe >/dev/null 2>&1; then
		if iptables -t mangle -A pwplus_probe -p tcp -j TPROXY \
		     --on-ip 127.0.0.1 --on-port 65500 --tproxy-mark 0x1 >/dev/null 2>&1; then
			iptables -t mangle -F pwplus_probe >/dev/null 2>&1 || true
			iptables -t mangle -X pwplus_probe >/dev/null 2>&1 || true
			echo 1 > "$_cache" 2>/dev/null || true
			return 0
		fi
		iptables -t mangle -F pwplus_probe >/dev/null 2>&1 || true
...
forget_caps() {
	rm -f "$PWPLUS_RUN"/caps.* 2>/dev/null || true
	return 0
}
...
pwplus_unlock() {
	rm -rf "$PWPLUS_RUN/lock" 2>/dev/null || true
	return 0
}
...
# else's.
tunnel_running() {
	pgrep -f "run -config $PWPLUS_CONFIG_JSON" >/dev/null 2>&1
}
